# Running Svelte Commerce on X-Cart

This storefront can talk to a [X-Cart](https://www.x-cart.com) backend through the `@misiki/x-cart-connector` connector.

> **Status: connector published, storefront wiring not included.**
> `@misiki/x-cart-connector` exists at version `0.5.0`, but this repo ships env wiring and an override module for
> Medusa, Saleor and Vendure only. Running X-Cart means adding those two pieces yourself — both
...
|                  |                            |
| :--------------- | :------------------------- |
| Connector        | `@misiki/x-cart-connector` |
| Version          | 0.5.0                      |
...
2. That is the whole switch — there is no file in this repo to edit. `vite.config.ts` resolves
   ...
   ```env
   PUBLIC_CONNECTOR='@misiki/x-cart-connector'
   ```

3. Give the connector its base URL in `.env`:

   ```env
   PUBLIC_X_CART_API_URL=https://your-store.example.com
   ```

   `src/lib/core/connectors/init.ts` derives this backend's variables from its connector name
   (`PUBLIC_X_CART_*`) and applies them at boot, in both hooks — the server one covers SSR, the client one covers the
   ...
   `PUBLIC_X_CART_API_KEY`, `PUBLIC_X_CART_API_SECRET`, `PUBLIC_X_CART_ACCESS_TOKEN`, `PUBLIC_X_CART_ACCESS_KEY`, `PUBLIC_X_CART_STORE_ID`, `PUBLIC_X_CART_CHANNEL_ID`. Unset ones are not passed, so they never overwrite a value the connector already
   holds.

4. What the wrapper does. `@misiki/x-cart-connector` already carries its own `setStaticStore`,
...
Services not wired for this platform: **vendor**. Pages that depend on them render their
...
- **`[x-cart] no native implementation for \`get /api/...\`` in the console** — the connector's
  REST guard caught an inherited Litekart path and answered it empty rather than requesting it.
  Each path is reported once. Harmless if that feature has no x-cart equivalent; if it does,
