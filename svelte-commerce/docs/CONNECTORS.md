...
Svelte Commerce talks to any backend through a **connector**: a small TypeScript package that maps
one platform's API onto a shared 43-service surface (`product`, `cart`, `checkout`, `order`,
`auth`, `address`, …). Nothing in the storefront imports a connector by name, and no file in it
names one: install a different connector package and the same storefront runs on a different
backend.
