# Running Svelte Commerce on Virto Commerce

This storefront can talk to a [Virto Commerce](https://virtocommerce.com) backend through the `@misiki/virto-connector` connector.

> **Status: connector published, storefront wiring not included.**
> `@misiki/virto-connector` exists at version `0.4.0`, but this repo ships env wiring and an override module for
> Medusa, Saleor and Vendure only. Running Virto Commerce means adding those two pieces yourself — both
...
|                  |                           |
| :--------------- | :------------------------ |
| Connector        | `@misiki/virto-connector` |
| Version          | 0.4.0                     |
...
Targeting this platform required choosing which API generation to wire against.
...
2. That is the whole switch — there is no file in this repo to edit. `vite.config.ts` resolves
   ...
   ```env
   PUBLIC_CONNECTOR='@misiki/virto-connector'
   ```

3. Give the connector its base URL in `.env`:

   ```env
   PUBLIC_VIRTO_COMMERCE_API_URL=https://your-store.example.com
   ```

   `src/lib/core/connectors/init.ts` derives this backend's variables from its connector name
   (`PUBLIC_VIRTO_COMMERCE_*`) and applies them at boot, in both hooks — the server one covers SSR, the client one covers the
   ...
   `PUBLIC_VIRTO_COMMERCE_API_KEY`, `PUBLIC_VIRTO_COMMERCE_API_SECRET`, `PUBLIC_VIRTO_COMMERCE_ACCESS_TOKEN`, `PUBLIC_VIRTO_COMMERCE_ACCESS_KEY`, `PUBLIC_VIRTO_COMMERCE_STORE_ID`, `PUBLIC_VIRTO_COMMERCE_CHANNEL_ID`, `PUBLIC_VIRTO_COMMERCE_CULTURE_NAME`, `PUBLIC_VIRTO_COMMERCE_CURRENCY_CODE`, `PUBLIC_VIRTO_COMMERCE_CATALOG_ID`. Unset ones are not passed, so they never overwrite a value the connector already
   holds.

4. What the wrapper does. `@misiki/virto-connector` already carries its own `setStaticStore`,
...
- **`[virto] no native implementation for \`get /api/...\`` in the console** — the connector's
  REST guard caught an inherited Litekart path and answered it empty rather than requesting it.
  Each path is reported once. Harmless if that feature has no virto equivalent; if it does,
