# Running Svelte Commerce on Shuup

This storefront can talk to a [Shuup](https://github.com/shuup/shuup) backend through the `@misiki/shuup-connector` connector.

> **Status: connector published, storefront wiring not included.**
> `@misiki/shuup-connector` exists at version `0.5.0`, but this repo ships env wiring and an override module for
> Medusa, Saleor and Vendure only. Running Shuup means adding those two pieces yourself — both
...
|                  |                                                |
| :--------------- | :--------------------------------------------- |
| Connector        | `@misiki/shuup-connector`                      |
| Version          | 0.5.0                                          |
| Service coverage | **19/43**                                      |
| Cart             | ✅                                             |
| Checkout         | ✅                                             |
| Auth             | ✅                                             |
| Known gaps       | coupon, review, page, country, state, wishlist |
...
2. That is the whole switch — there is no file in this repo to edit. `vite.config.ts` resolves
   ...
   ```env
   PUBLIC_CONNECTOR='@misiki/shuup-connector'
   ```

3. Give the connector its base URL in `.env`:

   ```env
   PUBLIC_SHUUP_API_URL=https://your-store.example.com
   ```

   `src/lib/core/connectors/init.ts` derives this backend's variables from its connector name
   (`PUBLIC_SHUUP_*`) and applies them at boot, in both hooks — the server one covers SSR, the client one covers the
   ...
   `PUBLIC_SHUUP_API_KEY`, `PUBLIC_SHUUP_API_SECRET`, `PUBLIC_SHUUP_ACCESS_TOKEN`, `PUBLIC_SHUUP_ACCESS_KEY`, `PUBLIC_SHUUP_STORE_ID`, `PUBLIC_SHUUP_CHANNEL_ID`. Unset ones are not passed, so they never overwrite a value the connector already
   holds.

4. What the wrapper does. `@misiki/shuup-connector` already carries its own `setStaticStore`,
...
Services not wired for this platform: **coupon, review, page, country, state, wishlist**. Pages that depend on them render their
...
- **`[shuup] no native implementation for \`get /api/...\`` in the console** — the connector's
  REST guard caught an inherited Litekart path and answered it empty rather than requesting it.
  Each path is reported once. Harmless if that feature has no shuup equivalent; if it does,
