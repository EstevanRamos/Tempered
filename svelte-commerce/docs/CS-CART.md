# Running Svelte Commerce on CS-Cart

This storefront can talk to a [CS-Cart](https://www.cs-cart.com) backend through the `@misiki/cs-cart-connector` connector.

> **Status: connector published, storefront wiring not included.**
> `@misiki/cs-cart-connector` exists at version `0.4.0`, but this repo ships env wiring and an override module for
> Medusa, Saleor and Vendure only. Running CS-Cart means adding those two pieces yourself — both
...
|                  |                                                           |
| :--------------- | :-------------------------------------------------------- |
| Connector        | `@misiki/cs-cart-connector`                               |
| Version          | 0.4.0                                                     |
| Service coverage | **20/43**                                                 |
| Cart             | ✅                                                        |
| Checkout         | ✅                                                        |
| Auth             | ⛔                                                        |
| Known gaps       | auth, address, country, state, currency, coupon, wishlist |
...
2. That is the whole switch — there is no file in this repo to edit. `vite.config.ts` resolves
   ...
   ```env
   PUBLIC_CONNECTOR='@misiki/cs-cart-connector'
   ```

3. Give the connector its base URL in `.env`:

   ```env
   PUBLIC_CS_CART_API_URL=https://your-store.example.com
   ```

   `src/lib/core/connectors/init.ts` derives this backend's variables from its connector name
   (`PUBLIC_CS_CART_*`) and applies them at boot, in both hooks — the server one covers SSR, the client one covers the
   ...
   `PUBLIC_CS_CART_API_KEY`, `PUBLIC_CS_CART_API_SECRET`, `PUBLIC_CS_CART_ACCESS_TOKEN`, `PUBLIC_CS_CART_ACCESS_KEY`, `PUBLIC_CS_CART_STORE_ID`, `PUBLIC_CS_CART_CHANNEL_ID`. Unset ones are not passed, so they never overwrite a value the connector already
   holds.

4. What the wrapper does. `@misiki/cs-cart-connector` already carries its own `setStaticStore`,
...
Services not wired for this platform: **auth, address, country, state, currency, coupon, wishlist**. Pages that depend on them render their
...
- **`[cs-cart] no native implementation for \`get /api/...\`` in the console** — the connector's
  REST guard caught an inherited Litekart path and answered it empty rather than requesting it.
  Each path is reported once. Harmless if that feature has no cs-cart equivalent; if it does,
