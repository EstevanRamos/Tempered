...
2. That is the whole switch — there is no file in this repo to edit. `vite.config.ts` resolves
   ...
   ```env
   PUBLIC_CONNECTOR='@misiki/prestashop-connector'
   ```

3. Give the connector its base URL in `.env`:

   ```env
   PUBLIC_PRESTASHOP_API_URL=https://your-store.example.com
   ```

   `src/lib/core/connectors/init.ts` derives this backend's variables from its connector name
   (`PUBLIC_PRESTASHOP_*`) and applies them at boot, in both hooks — the server one covers SSR, the client one covers the
...
4. What the wrapper does. `@misiki/prestashop-connector` already carries its own `setStaticStore`,
