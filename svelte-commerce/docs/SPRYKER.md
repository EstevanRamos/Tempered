# Running Svelte Commerce on Spryker

This storefront can talk to a [Spryker](https://spryker.com) backend through the `@misiki/spryker-connector` connector.

> **Status: connector published, storefront wiring not included.**
> `@misiki/spryker-connector` exists at version `0.4.0`, but this repo ships env wiring and an override module for
> Medusa, Saleor and Vendure only. Running Spryker means adding those two pieces yourself — both
...
|                  |                             |
| :--------------- | :-------------------------- |
| Connector        | `@misiki/spryker-connector` |
| Version          | 0.4.0                       |
...
2. That is the whole switch — there is no file in this repo to edit. `vite.config.ts` resolves
   ...
   ```env
   PUBLIC_CONNECTOR='@misiki/spryker-connector'
   ```

3. Give the connector its base URL in `.env`:

   ```env
   PUBLIC_SPRYKER_API_URL=https://your-store.example.com
   ```

   `src/lib/core/connectors/init.ts` derives this backend's variables from its connector name
   (`PUBLIC_SPRYKER_*`) and applies them at boot, in both hooks — the server one covers SSR, the client one covers the
   ...
   `PUBLIC_SPRYKER_API_KEY`, `PUBLIC_SPRYKER_API_SECRET`, `PUBLIC_SPRYKER_ACCESS_TOKEN`, `PUBLIC_SPRYKER_ACCESS_KEY`, `PUBLIC_SPRYKER_STORE_ID`, `PUBLIC_SPRYKER_CHANNEL_ID`, `PUBLIC_SPRYKER_LOCALE`, `PUBLIC_SPRYKER_ANONYMOUS_CUSTOMER_ID`, `PUBLIC_SPRYKER_NAVIGATION_KEY`. Unset ones are not passed, so they never overwrite a value the connector already
   holds.

4. What the wrapper does. `@misiki/spryker-connector` already carries its own `setStaticStore`,
...
- **`[spryker] no native implementation for \`get /api/...\`` in the console** — the connector's
  REST guard caught an inherited Litekart path and answered it empty rather than requesting it.
  Each path is reported once. Harmless if that feature has no spryker equivalent; if it does,
