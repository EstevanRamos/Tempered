...
2. That is the whole switch — there is no file in this repo to edit. `vite.config.ts` resolves
   ...
   ```env
   PUBLIC_CONNECTOR='@misiki/sylius-connector'
   ```
...
3. Give the connector its base URL in `.env`:
   ...
   (`PUBLIC_SYLIUS_*`) and applies them at boot, in both hooks — the server one covers SSR, the client one covers the
...
4. What the wrapper does. `@misiki/sylius-connector` already carries its own `setStaticStore`,
