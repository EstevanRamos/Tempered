# Running Svelte Commerce on Sharetribe

This storefront can talk to a [Sharetribe](https://www.sharetribe.com) backend through the `@misiki/sharetribe-connector` connector.

> **Status: connector published, storefront wiring not included.**
> `@misiki/sharetribe-connector` exists at version `0.4.0`, but this repo ships env wiring and an override module for
> Medusa, Saleor and Vendure only. Running Sharetribe means adding those two pieces yourself — both
...
|                  |                                                 |
| :--------------- | :---------------------------------------------- |
| Connector        | `@misiki/sharetribe-connector`                  |
| Version          | 0.4.0                                           |
| Service coverage | **21/43**                                       |
| Cart             | —                                               |
| Checkout         | ✅                                              |
| Auth             | ✅                                              |
| Known gaps       | cart, coupon, address, country, state, wishlist |
...
A marketplace, not a webshop. The mapping is `listing` to `Product` and `transaction` to `Order`. There is deliberately no cart, no coupon and no category tree — expect the storefront's cart and coupon surfaces to be inert.
...
Services not wired for this platform: **cart, coupon, address, country, state, wishlist**. Pages that depend on them render their
...
## Open question for Sharetribe maintainers

Is `listing` to `Product` the mapping you would recommend, or should a storefront model transactions as the primary entity?
...
- **`[sharetribe] no native implementation for \`get /api/...\`` in the console** — the connector's
  REST guard caught an inherited Litekart path and answered it empty rather than requesting it.
  Each path is reported once. Harmless if that feature has no sharetribe equivalent; if it does,
