# Running Svelte Commerce on EverShop

This storefront can talk to a [EverShop](https://evershop.io) backend through the `@misiki/evershop-connector` connector.

> **Status: connector published, storefront wiring not included.**
> `@misiki/evershop-connector` exists at version `0.4.0`, but this repo ships env wiring and an override module for
> Medusa, Saleor and Vendure only. Running EverShop means adding those two pieces yourself — both
...
|                  |                              |
| :--------------- | :--------------------------- |
| Connector        | `@misiki/evershop-connector` |
| Version          | 0.4.0                        |
| Service coverage | **25/43**                    |
| Cart             | ✅                           |
| Checkout         | ✅                           |
| Auth             | ✅                           |
| Known gaps       | review, wishlist, vendor     |
...
1. Install the connector in place of the stock one:
   ...
   npm i @misiki/evershop-connector
   ```
...
2. That is the whole switch — there is no file in this repo to edit. `vite.config.ts` resolves
   ...
   ```env
   PUBLIC_CONNECTOR='@misiki/evershop-connector'
   ```

3. Give the connector its base URL in `.env`:

   ```env
   PUBLIC_EVERSHOP_API_URL=https://your-store.example.com
   ```

   `src/lib/core/connectors/init.ts` derives this backend's variables from its connector name
   (`PUBLIC_EVERSHOP_*`) and applies them at boot, in both hooks — the server one covers SSR, the client one covers the
   ...
   `PUBLIC_EVERSHOP_API_KEY`, `PUBLIC_EVERSHOP_API_SECRET`, `PUBLIC_EVERSHOP_ACCESS_TOKEN`, `PUBLIC_EVERSHOP_ACCESS_KEY`, `PUBLIC_EVERSHOP_STORE_ID`, `PUBLIC_EVERSHOP_CHANNEL_ID`. Unset ones are not passed, so they never overwrite a value the connector already
   holds.

4. What the wrapper does. `@misiki/evershop-connector` already carries its own `setStaticStore`,
...
Services not wired for this platform: **review, wishlist, vendor**. Pages that depend on them render their
...
- **`[evershop] no native implementation for \`get /api/...\`` in the console** — the connector's
  REST guard caught an inherited Litekart path and answered it empty rather than requesting it.
  Each path is reported once. Harmless if that feature has no evershop equivalent; if it does,
