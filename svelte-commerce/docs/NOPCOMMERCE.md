# Running Svelte Commerce on nopCommerce

This storefront can talk to a [nopCommerce](https://www.nopcommerce.com) backend through the `@misiki/nopcommerce-connector` connector.

> **Status: connector published, storefront wiring not included.**
> `@misiki/nopcommerce-connector` exists at version `0.5.0`, but this repo ships env wiring and an override module for
> Medusa, Saleor and Vendure only. Running nopCommerce means adding those two pieces yourself — both
...
|                  |                                 |
| :--------------- | :------------------------------ |
| Connector        | `@misiki/nopcommerce-connector` |
| Version          | 0.5.0                           |
| Service coverage | **24/43**                       |
| Cart             | ✅                              |
| Checkout         | ✅                              |
| Auth             | ✅                              |
| Known gaps       | coupon, currency                |
...
Core ships no REST API. This connector targets the **API-Frontend plugin** contract (v4.50.09, 2022), so that plugin must be installed on your instance.
...
2. That is the whole switch — there is no file in this repo to edit. `vite.config.ts` resolves
   ...
   ```env
   PUBLIC_CONNECTOR='@misiki/nopcommerce-connector'
   ```

3. Give the connector its base URL in `.env`:

   ```env
   PUBLIC_NOPCOMMERCE_API_URL=https://your-store.example.com
   ```

   `src/lib/core/connectors/init.ts` derives this backend's variables from its connector name
   (`PUBLIC_NOPCOMMERCE_*`) and applies them at boot, in both hooks — the server one covers SSR, the client one covers the
   ...
   `PUBLIC_NOPCOMMERCE_API_KEY`, `PUBLIC_NOPCOMMERCE_API_SECRET`, `PUBLIC_NOPCOMMERCE_ACCESS_TOKEN`, `PUBLIC_NOPCOMMERCE_ACCESS_KEY`, `PUBLIC_NOPCOMMERCE_STORE_ID`, `PUBLIC_NOPCOMMERCE_CHANNEL_ID`. Unset ones are not passed, so they never overwrite a value the connector already
   holds.

4. What the wrapper does. `@misiki/nopcommerce-connector` already carries its own `setStaticStore`,
...
Services not wired for this platform: **coupon, currency**. Pages that depend on them render their
...
## Open question for nopCommerce maintainers

If there is a published API contract newer than API-Frontend v4.50.09, please point us at it.
...
- **`[nopcommerce] no native implementation for \`get /api/...\`` in the console** — the connector's
  REST guard caught an inherited Litekart path and answered it empty rather than requesting it.
  Each path is reported once. Harmless if that feature has no nopcommerce equivalent; if it does,
