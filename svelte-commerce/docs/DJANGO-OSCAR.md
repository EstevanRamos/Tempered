# Running Svelte Commerce on Django Oscar

This storefront can talk to a [Django Oscar](https://github.com/django-oscar/django-oscar) backend through the `@misiki/oscar-connector` connector.

> **Status: connector published, storefront wiring not included.**
> `@misiki/oscar-connector` exists at version `0.5.0`, but this repo ships env wiring and an override module for
> Medusa, Saleor and Vendure only. Running Django Oscar means adding those two pieces yourself — both
...
|                  |                                                                                 |
| :--------------- | :------------------------------------------------------------------------------ |
| Connector        | `@misiki/oscar-connector`                                                       |
| Version          | 0.5.0                                                                           |
| Service coverage | **15/43**                                                                       |
| Cart             | ✅                                                                              |
| Checkout         | ✅                                                                              |
| Auth             | ✅                                                                              |
| Known gaps       | coupon, review, search, page, payment-method, currency, state, wishlist, vendor |
...
Wired against **django-oscar-api**, which is a separate package from Oscar core. It must be installed and routed on your instance.
...
2. That is the whole switch — there is no file in this repo to edit. `vite.config.ts` resolves
   ...
   ```env
   PUBLIC_CONNECTOR='@misiki/oscar-connector'
   ```

3. Give the connector its base URL in `.env`:

   ```env
   PUBLIC_DJANGO_OSCAR_API_URL=https://your-store.example.com
   ```

   `src/lib/core/connectors/init.ts` derives this backend's variables from its connector name
   (`PUBLIC_DJANGO_OSCAR_*`) and applies them at boot, in both hooks — the server one covers SSR, the client one covers the
   ...
   `PUBLIC_DJANGO_OSCAR_API_KEY`, `PUBLIC_DJANGO_OSCAR_API_SECRET`, `PUBLIC_DJANGO_OSCAR_ACCESS_TOKEN`, `PUBLIC_DJANGO_OSCAR_ACCESS_KEY`, `PUBLIC_DJANGO_OSCAR_STORE_ID`, `PUBLIC_DJANGO_OSCAR_CHANNEL_ID`. Unset ones are not passed, so they never overwrite a value the connector already
   holds.

4. What the wrapper does. `@misiki/oscar-connector` already carries its own `setStaticStore`,
...
Services not wired for this platform: **coupon, review, search, page, payment-method, currency, state, wishlist, vendor**. Pages that depend on them render their
...
## Open question for Django Oscar maintainers

Is a headless storefront a supported use case for django-oscar-api? Worth stating either way in your docs.
...
- **`[oscar] no native implementation for \`get /api/...\`` in the console** — the connector's
  REST guard caught an inherited Litekart path and answered it empty rather than requesting it.
  Each path is reported once. Harmless if that feature has no oscar equivalent; if it does,
