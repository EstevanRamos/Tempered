# Running Svelte Commerce on Shopware

This storefront can talk to a [Shopware](https://www.shopware.com) backend through the `@misiki/shopware-connector` connector.

> **Status: connector published, storefront wiring not included.**
> `@misiki/shopware-connector` exists at version `0.4.0`, but this repo ships env wiring and an override module for
> Medusa, Saleor and Vendure only. Running Shopware means adding those two pieces yourself — both
...
|                  |                              |
| :--------------- | :--------------------------- |
| Connector        | `@misiki/shopware-connector` |
| Version          | 0.4.0                        |
...
The Store API is one of the cleanest headless surfaces of the set. Cart and customer session ride the `sw-context-token` header, which the connector captures and replays like a cookie.
...
2. That is the whole switch — there is no file in this repo to edit. `vite.config.ts` resolves
   ...
   ```env
   PUBLIC_CONNECTOR='@misiki/shopware-connector'
   ```

3. Give the connector its base URL in `.env`:

   ```env
   PUBLIC_SHOPWARE_API_URL=https://your-store.example.com
   ```

   `src/lib/core/connectors/init.ts` derives this backend's variables from its connector name
   (`PUBLIC_SHOPWARE_*`) and applies them at boot, in both hooks — the server one covers SSR, the client one covers the
   ...
   `PUBLIC_SHOPWARE_API_KEY`, `PUBLIC_SHOPWARE_API_SECRET`, `PUBLIC_SHOPWARE_ACCESS_TOKEN`, `PUBLIC_SHOPWARE_ACCESS_KEY`, `PUBLIC_SHOPWARE_STORE_ID`, `PUBLIC_SHOPWARE_CHANNEL_ID`. Unset ones are not passed, so they never overwrite a value the connector already
   holds.

4. What the wrapper does. `@misiki/shopware-connector` already carries its own `setStaticStore`,
...
Services not wired for this platform: **coupon**. Pages that depend on them render their
...
## Open question for Shopware maintainers

Is there a Store API route for promotion/voucher _lookup_ (not just applying one to a cart)? We found apply/remove but no way to list or validate a code standalone.
...
- **`[shopware] no native implementation for \`get /api/...\`` in the console** — the connector's
  REST guard caught an inherited Litekart path and answered it empty rather than requesting it.
  Each path is reported once. Harmless if that feature has no shopware equivalent; if it does,
