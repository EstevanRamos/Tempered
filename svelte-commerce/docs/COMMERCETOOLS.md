...
2. That is the whole switch — there is no file in this repo to edit. `vite.config.ts` resolves
   ...
   ```env
   PUBLIC_CONNECTOR='@misiki/commercetools-connector'
   ```
...
3. Give the connector its base URL in `.env`:
   ...
   (`PUBLIC_COMMERCETOOLS_*`) and applies them at boot, in both hooks — the server one covers SSR, the client one covers the
...
4. What the wrapper does. `@misiki/commercetools-connector` already carries its own `setStaticStore`,
...
- **`[commercetools] no native implementation for \`get /api/...\`` in the console** — the connector's
  REST guard caught an inherited Litekart path and answered it empty rather than requesting it.
  Each path is reported once. Harmless if that feature has no commercetools equivalent; if it does,
